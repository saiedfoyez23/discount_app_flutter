...
class RiderSubscriptionController extends GetxController {
  ...
  RiderSubscriptionController({required this.context});
  ...
  Future<void> createPaymentController({
    ...
    BaseApiUtils.post(
      ...
      onSuccess: (e,data) async {
        ...
        Get.off(()=>RiderPaymentView(paymentUrl: data["data"]["url"]),duration: const Duration(milliseconds: 100),preventDuplicates: false);
      },
      onFail: (e,data) {
        MessageSnackBarWidget.errorSnackBarWidget(context: context, message: e);
        isSubmit.value = false;
